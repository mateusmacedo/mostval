...
interface TestPayload extends TFlexible<{ value: string }> {
  value: string;
}

interface TestMetadata extends TFlexible<{ id: string }> {
  id: string;
  schema: string;
  type: string;
  timestamp: number;
...
class TestMessage extends Message<TestPayload, TestMetadata> {
  constructor(value: string, metadata: TestMetadata) {
    super({ value }, metadata);
  }
}

describe('InMemoryMessageStore', () => {
  let store: InMemoryMessageStore<Message<TestPayload, TestMetadata>>;
  ...
  describe('remove', () => {
    ...
    it('should handle null/undefined metadata properties', () => {
      ...
      Object.defineProperty(messageWithNullMetadata, 'metadata', {
        value: null,
      });
