import { BaseMetadata, TFlexible, InMemoryMessageStore } from '@mostval/common';
import {
...
describe('UserService', () => {
  ...
  describe('createUser', () => {
    ...
    it('should throw an error when factory fails', async () => {
      ...
      await expect(service.createUser(defaultUserProps)).rejects.toThrow(
  ...
  describe('findUser', () => {
    ...
    it('should throw an error when repository fails', async () => {
      ...
      await expect(service.findUser(criteria)).rejects.toThrow(
        'User finding failed'
      );
    });
  });
  ...
  describe('deleteUser', () => {
    ...
    it('should throw an error when repository fails', async () => {
      ...
      await expect(service.deleteUser(criteria)).rejects.toThrow(
  ...
  describe('updateUserCredentials', () => {
    ...
    const metadata: TFlexible<BaseMetadata> = {
      id: '1',
      schema: 'user',
    ...
    const command = new ChangeUserCredentialsCommand(
      updatedCredentials,
      metadata
    );
    ...
    it('should throw an error when user is not found', async () => {
      ...
      await expect(service.updateUserCredentials(command)).rejects.toThrow(
        'User not found'
      );
    });
  });
