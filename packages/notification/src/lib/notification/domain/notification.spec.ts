...
describe('Notification', () => {
  ...
  const mockIdentity: (
    id: string,
    version: number
  ) => TIdentity<string, number> = (id, version) => ({
    id,
    version,
  });

  const mockTimestamps: (createdAt: Date, updatedAt: Date) => ITimestamper = (
    createdAt,
    updatedAt
  ) => ({
    createdAt,
  ...
  it('should compare two notifications correctly - different case', () => {
    ...
    const notification2 = new Notification(
      mockProps(mockChannels, 'Different content', NotificationStatus.CREATED),
      mockIdentity('notification-1', 1),
      mockTimestamps(mockDate, mockDate)
    );
